...
export async function GET(
  ...
  try {
    ...
    let finalWhere: any = { doctorName: name, ...surgeryRange }

    const filtersParam = url.searchParams.get('filters')
    let doctorPaidFilter: any = null
    ...
    if (filtersParam) {
      try {
          ...
          for (const f of parsedFilters) {
            ...
            if (field === 'hospital') {
              if (Array.isArray(value) && value.length > 0) {
                filterConditions.push({ hospitalName: { in: value } })
              }
            } else if (field === 'status') {
              if (Array.isArray(value) && value.length > 0) {
                filterConditions.push({ status: { in: value } })
              }
            } else if (field === 'doctorPayoutStatus') {
              if (Array.isArray(value) && value.length > 0) {
                filterConditions.push({ doctorPayoutStatus: { in: value } })
              }
            ...
            // ── numberRange / between ────────────────────────────────────
              ...
              if (max != null) range.lte = max
              if (Object.keys(range).length > 0) {
                filterConditions.push({ doctorCharges: range })
              }
            } else if (field === 'doctorAmountPending') {
              const { min, max } = value as { min: number | null; max: number | null }
              const range: any = {}
              if (min != null) range.gte = min
              if (max != null) range.lte = max
              if (Object.keys(range).length > 0) {
                filterConditions.push({ doctorAmountPending: range })
              }
            } else if (field === 'doctorPaid') {
              doctorPaidFilter = value
            }
          }
        ...
        console.error('Error parsing doctor details filters:', err)
      }
    }
    ...
    if (doctorPaidFilter) {
      const { min, max } = doctorPaidFilter as { min: number | null; max: number | null }
      cases = cases.filter(c => {
        if (min != null && c.doctorPaid < min) return false
        if (max != null && c.doctorPaid > max) return false
        return true
      })
    }
