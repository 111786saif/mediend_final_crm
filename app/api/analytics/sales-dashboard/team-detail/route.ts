...
import { Prisma, UserRole, CaseStage } from '@/generated/prisma/client'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const teamOpdWhere: Prisma.LeadWhereInput = {
      bdId: { in: allUserIds },
      AND: [
        {
          OR: [
        ...
        {
          OR: [
            { caseStage: { in: [CaseStage.OPD_DONE, CaseStage.CASH_OPD_DONE] } },
            { status: { in: ['OPD Done', 'opd done', 'OPD DONE', '11', 'opd_done'] } },
          ],
        },
      ],
    }
    ...
    const [allLeads, opdLeads, completedLeads, categoryLeads, categoryIpd] = await Promise.all([
      prisma.lead.groupBy({
        by: ['bdId'],
    ...
    const opdMap = new Map(opdLeads.map((r) => [r.bdId, r]))
    const ipdMap = new Map(completedLeads.map((r) => [r.bdId, r]))

    const ipdByMonth = await prisma.$queryRaw<{ month: string; bdId: string; bdName: string; count: number }[]>`
      SELECT
    ...
    const members = bdMembers.map((m) => {
      ...
      const opd = opdMap.get(m.userId)?._count.id ?? 0
    ...
    const tlOpd = opdMap.get(managerUserId)?._count.id ?? 0
    const tlIpd = ipdMap.get(managerUserId)?._count.id ?? 0
    if (tlLeads > 0 || tlOpd > 0 || tlIpd > 0) {
    ...
    for (const unit of nestedTeamUnits) {
      ...
      const opd = opdMap.get(unit.userId)?._count.id ?? 0
      const ipd = ipdMap.get(unit.userId)?._count.id ?? 0
      if (leads === 0 && opd === 0 && ipd === 0) continue
    ...
    const totalOpd = members.reduce((s, m) => s + (m.opdDone ?? 0), 0)
