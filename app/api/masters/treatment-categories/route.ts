...
export async function GET(request: NextRequest) {
  ...
  const where: Prisma.TreatmentCategoryMasterWhereInput = {}
  if (!includeInactive) {
    where.isActive = true
  }
  if (search) {
  ...
  const items = await prisma.treatmentCategoryMaster.findMany({
    where,
    orderBy: { name: 'asc' },
    take: 500,
  })
...
export async function POST(request: NextRequest) {
  ...
  try {
    const created = await prisma.treatmentCategoryMaster.create({
      data: { name: parsed.data.name.trim() },
    })
    return successResponse({ item: created })
  } catch (e) {
    if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
      return errorResponse('A treatment category with this name already exists', 409)
    }
    throw e
  }
}
