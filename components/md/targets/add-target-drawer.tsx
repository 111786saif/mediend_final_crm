  ...
  return (
    <Sheet open={open} onOpenChange={(o) => !o && handleClose()}>
      <SheetContent
        ...
        className={cn(
          'flex flex-col gap-0 p-0 z-[100] overflow-hidden min-h-0',
          '[&>button]:hidden',
          isMobile
            ? 'h-[min(92dvh,100%)] max-h-[min(92dvh,100%)] w-full max-w-full rounded-t-2xl border-0 pt-0'
            : 'h-full max-h-dvh w-full max-w-full sm:max-w-none md:w-[min(92vw,800px)] md:max-w-[min(92vw,800px)]'
        )}
      >
        {isMobile ? (
          <div className="mx-auto mt-2 h-1.5 w-12 shrink-0 rounded-full bg-muted" aria-hidden />
        ) : null}
        <SheetHeader className="flex shrink-0 flex-row items-center gap-3 border-b space-y-0 px-4 pt-6 pb-3 sm:gap-4 sm:px-6 sm:pt-6 sm:pb-4">
          {step === 2 && (
        ...
        {isMobile ? (
          <div className="min-h-0 flex-1 overflow-y-auto touch-pan-y overscroll-contain [-webkit-overflow-scrolling:touch]">
            {addTargetScrollInner}
          </div>
        ) : (
          <ScrollArea className="min-h-0 flex-1">{addTargetScrollInner}</ScrollArea>
        )}

        {step === 2 && (
          <SheetFooter
            className={cn(
              'shrink-0 border-t px-4 pt-3 sm:px-6 sm:pt-4',
              isMobile
                ? 'pb-[max(1rem,calc(4.5rem+env(safe-area-inset-bottom)))]'
                : 'pb-4'
            )}
          >
            <Button
