...
const docsDir = path.join(process.cwd(), 'app', 'documentation');
const compDir = path.join(docsDir, 'components');

// Generate docs-components.tsx
const docsComponents = 'use client'
...
export function APITable({ routes }) {
  const methodColor = (m) => {
    ...
    if (m === 'PAGE') return 'bg-indigo-100 text-indigo-700'
...
export function Callout({ type = 'info', title, children }) {
  const colors = { info: 'border-blue-500 bg-blue-50', warning: 'border-amber-500 bg-amber-50', success: 'border-emerald-500 bg-emerald-50' }
  return (
    <div className={'rounded-lg border-l-4 p-4 my-4 ' + colors[type]}>
      <div className="text-base font-semibold mb-1 text-gray-900 dark:text-white">{title}</div>
...
fs.writeFileSync(path.join(compDir, 'docs-components.tsx'), docsComponents)
console.log('Created docs-components.tsx')
